...
// Alleen slug en naam, voor plekken die enkel links nodig hebben — zoals de
// footer op elke pagina. De volledige records bevatten hele paginateksten en
// wegen samen bijna 300 KB; die horen alleen in de landingspagina's zelf.
const linkTs = (naam, records) => `// Auto-generated by scripts/generate-landing-data.mjs. Do not edit by hand.
export interface LandingLink { slug: string; name: string; }

export const ${naam}: LandingLink[] = ${JSON.stringify(records.map((r) => ({ slug: r.slug, name: r.name })), null, 2)};
`;

const outDir = path.resolve("src/data");
fs.writeFileSync(path.join(outDir, "cities.ts"), cityTs);
fs.writeFileSync(path.join(outDir, "industries.ts"), indTs);
fs.writeFileSync(path.join(outDir, "cityLinks.ts"), linkTs("cityLinks", cityRecords));
fs.writeFileSync(path.join(outDir, "industryLinks.ts"), linkTs("industryLinks", industryRecords));
