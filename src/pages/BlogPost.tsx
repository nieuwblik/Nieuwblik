...
const BlogPost = () => {
  ...
  const formatContent = (content: string) => {
    ...
    return sections.map((section, index) => {
      ...
      if (section.startsWith('# ')) {
        return <h1 key={index} className="text-3xl md:text-4xl font-bold mb-6 mt-8 first:mt-0">
            {section.replace('# ', '')}
          </h1>;
      }
      ...
      if (section.startsWith('## ')) {
        ...
        const id = text.toLowerCase().replace(/[^a-z0-9]+/g, '-');
        return <h2 key={index} id={id} className="text-2xl md:text-3xl font-bold mb-4 mt-8 scroll-mt-24">
            {text}
          </h2>;
      }
      ...
      if (section.startsWith('### ')) {
        const text = section.replace('### ', '');
        const id = text.toLowerCase().replace(/[^a-z0-9]+/g, '-');
        return <h3 key={index} id={id} className="text-xl md:text-2xl font-semibold mb-3 mt-6 scroll-mt-24">
            {text}
          </h3>;
      }
      ...
      // Blockquote
      if (section.startsWith('> ')) {
        return <blockquote key={index} className="border-l-4 border-accent pl-4 italic text-base md:text-lg my-6 text-muted-foreground">
            {section.replace('> ', '')}
          </blockquote>;
      }
      ...
      if (section.includes('\n- ')) {
        ...
        return <ul key={index} className="list-disc list-inside space-y-2 my-4 text-base">
            ...
            return <li key={i} className="text-foreground font-light">
      ...
      if (section.match(/^[❌✅]/m)) {
        ...
        return <ul key={index} className="space-y-3 my-6">
            {lines.map((line, i) => {
            const text = line.slice(2);
            const parts = text.split('**');
            return <li key={i} className="text-lg flex items-start gap-3">
  ...
  const TableOfContents = () => <nav className="sticky top-32 max-h-[calc(100vh-10rem)] overflow-y-auto">
      <h3 className="text-lg font-semibold mb-4">
        Inhoudsopgave
      </h3>
      <ul className="space-y-2 text-sm">
        {tocItems.map(item => <li key={item.id} className={item.level === 3 ? "ml-4" : ""}>
            <button onClick={() => scrollToSection(item.id)} className={`text-left w-full py-1 px-2 rounded transition-colors ${activeSection === item.id ? "text-accent font-semibold bg-accent/10" : "text-muted-foreground hover:text-foreground hover:bg-secondary"}`}>
              {item.text}
            </button>
          </li>)}
      </ul>
  ...
  return <div className="min-h-screen bg-background">
      {post && <SEOHead title={`${post.title.nl} | Blog - Nieuwblik`} description={post.excerpt.nl} keywords="webdesign, SEO, conversie, digitale marketing, website optimalisatie" canonicalUrl={`https://www.nieuwblik.com/blog/${slug}`} structuredData={structuredData || undefined} />}
      <Navigation />
      ...
      <div className="fixed top-0 left-0 right-0 h-1 bg-secondary z-50">
        <div className="h-full bg-accent transition-all duration-150" style={{
      ...
      <section className="pb-8">
        <div className="container mx-auto px-6">
          <Link to="/blog" className="inline-flex items-center gap-2 text-accent hover:gap-3 transition-all font-semibold">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 max-w-7xl mx-auto">
            ...
            <div className="lg:col-span-2">
              <div className="max-w-3xl">
                ...
                {post.image && <div className="mb-12 -mx-6 md:mx-0">
                    <img src={post.image} alt={post.title.nl} className="w-full h-[400px] md:h-[500px] object-cover rounded-none md:rounded-lg" loading="eager" />
                  </div>}
                ...
                <div className="mt-16 pt-8 border-t border-border">
                  <div className="flex items-center gap-4">
                    <img src={justinImg} alt="Justin Slok - Oprichter Nieuwblik" loading="lazy" decoding="async" width="64" height="64" className="w-16 h-16 rounded-full object-cover" />
                    <div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-accent text-accent-foreground p-8 rounded-lg sticky top-32">
                <div className="flex items-center gap-4 mb-6">
                  <img src={justinImg} alt="Justin Slok - Nieuwblik" loading="lazy" className="w-20 h-20 rounded-full object-cover border-4 border-accent-foreground/20" />
                  <div>
                ...
                <div className="space-y-4">
                  <a href="tel:+31646253607" className="w-full block">
                  ...
                  <a href="https://wa.me/31646253607" target="_blank" rel="noopener noreferrer" className="w-full block">
